...
1. Update **Azure AD** (or Azure AD B2C) **App Registration** with URIs you have just obtained
...
1. Select the **App Registrations** blade on the left, then find and select the web app that you have registered in the previous tutorial (`ExpressWebApp`).
...
1. Find the key `redirectUri` and replace the existing value with the Redirect URI for `ExpressWebApp` app. For example, `https://msal-nodejs-webapp1.azurewebsites.net/redirect`.
1. Find the key `postLogoutRedirectUri` and replace the existing value with the base address of the `ExpressWebApp` project (by default `https://msal-nodejs-webapp1.azurewebsites.net/redirect/`).
