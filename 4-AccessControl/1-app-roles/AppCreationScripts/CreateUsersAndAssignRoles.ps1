...
Function CreateRolesUsersAndRoleAssignments
{
    ...
    $tenantName = $verifiedDomainName
     ...
     Write-Host "get the AAD application (msal-node-webapp)"
    $app = Get-MgApplication -Filter "DisplayName eq 'msal-node-webapp'" 
       ...
       $assignRole = New-MgUserAppRoleAssignment -Userid $newUser.Id -PrincipalId $newUser.Id -ResourceId $servicePrincipal.Id -AppRoleID $TaskUser.Id
        ...
        {
            Write-Host "Couldn't find application (msal-node-webapp)"  -BackgroundColor Red
        }
}
