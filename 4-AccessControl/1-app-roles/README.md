...
In [appSettings.json](./App/appSettings.json), we create an access matrix that defines the required roles and allowed HTTP methods for each route that we like to grant role-based access:
...
Then, in [router.js](./App/routes/router.js), we create an instance of the [AuthProvider]() class with the `appSettings.json` passed in constructor. The `authProvider` object exposes the middleware we can use to protect our app routes:
