...
Function ConfigureApplications
{
   ...
   $clientAadApplication = New-MgApplication -DisplayName "msal-node-webapp" `
    ...
    # Update config file for 'client'
    # $configFile = $pwd.Path + "\..\App\app.js"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\App\app.js"))
    
    $dictionary = @{ "Enter_the_ObjectId_of_GroupAdmin" = $GroupAdmin.Id;"Enter_the_ObjectId_of_GroupMember" = $GroupMember.Id };
