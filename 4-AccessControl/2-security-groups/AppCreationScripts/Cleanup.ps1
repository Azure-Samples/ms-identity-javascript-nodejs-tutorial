...
# if($null -ne (Get-AzureADGroup -SearchString GroupAdmin)) {
#   Remove-AzureADGroup -ObjectId (Get-AzureADGroup -SearchString GroupAdmin).objectId
# }
Write-Host "You may want to remove the security group 'GroupMember' if it was created to test this sample only."

# if($null -ne (Get-AzureADGroup -SearchString GroupMember)) {
#   Remove-AzureADGroup -ObjectId (Get-AzureADGroup -SearchString GroupMember).objectId
# }
