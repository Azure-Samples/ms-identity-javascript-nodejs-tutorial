...
Function ConfigureApplications
{
   ...
   $serviceAadApplication = New-MgApplication -DisplayName "msal-node-webapp" `
                                                       -Web `
                                                       @{ `
    ...
    Write-Host "Creating the service application (msal-node-webapp)"
    ...
    $serviceKeyCredentials = Update-MgApplication -ApplicationId $currentAppObjectId `
    ...
    Write-Host "Done creating the service application (msal-node-webapp)"
    ...
    Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>msal-node-webapp</a></td></tr>" -Path createdApps.html
    ...
    Write-Host "Successfully registered and configured that app registration for 'msal-node-webapp' at `n $servicePortalUrl" -ForegroundColor Green 
    
    # Update config file for 'service'
    # $configFile = $pwd.Path + "\..\APP\authConfig.js"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\APP\authConfig.js"))
    
    $dictionary = @{ "Enter_the_Application_Id_Here" = $serviceAadApplication.AppId;"Enter_the_Tenant_Id_Here" = $tenantId;"Enter_the_Client_Secret_Here" = $serviceAppKey;"YOUR_CERT_THUMBPRINT" = $thumbprint;"PATH_TO_YOUR_PRIVATE_KEY_FILE" = "../AppCreationScripts/"+$certificateName+".key" };
