...
#> 
   ...
   Write-Host "Creating the AAD application (ExpressWebApi-c3s2)"
   # create the application 
   $ServiceAadApplication = New-AzureADApplication -DisplayName "ExpressWebApi-c3s2" `
    ...
    if ($scopes.Count -ge 0) 
    {
        ...
        else 
        {
            ...
            $scope = CreateScope -value "access_as_user"  `
                -userConsentDisplayName "Access ExpressWebApi-c3s2"  `
                -userConsentDescription "Allow the application to access ExpressWebApi-c3s2 on your behalf."  `
                -adminConsentDisplayName "Access ExpressWebApi-c3s2"  `
   ...
   Write-Host "Done creating the Service application (ExpressWebApi-c3s2)"
   ...
   Add-Content -Value "<tr><td>Service</td><td>$currentAppId</td><td><a href='$ServicePortalUrl'>ExpressWebApi-c3s2</a></td></tr>" -Path createdApps.html


   # Create the Client AAD application
   Write-Host "Creating the AAD application (ExpressWebApp-c3s2)"
   ...
   $ClientAppKey = $pw
   # create the application 
   $ClientAadApplication = New-AzureADApplication -DisplayName "ExpressWebApp-c3s2" `
                                                  -HomePage "http://localhost:4000/" `
                                                  -ReplyUrls "http://localhost:4000/redirect" `
                                                  -IdentifierUris "https://$tenantName/ExpressWebApp-c3s2" `
   ...
   Write-Host "Done creating the Client application (ExpressWebApp-c3s2)"

   # URL of the AAD application in the Azure portal
   # Future? $ClientPortalUrl = "https://portal.azure.com/#@"+$tenantName+"/blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/Overview/appId/"+$ClientAadApplication.AppId+"/objectId/"+$ClientAadApplication.ObjectId+"/isMSAApp/"
   $ClientPortalUrl = "https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/CallAnAPI/appId/"+$ClientAadApplication.AppId+"/objectId/"+$ClientAadApplication.ObjectId+"/isMSAApp/"
   Add-Content -Value "<tr><td>Client</td><td>$currentAppId</td><td><a href='$ClientPortalUrl'>ExpressWebApp-c3s2</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]

   # Add Required Resources Access (from 'Client' to 'Service')
   Write-Host "Getting access from 'Client' to 'Service'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "ExpressWebApi-c3s2" `
